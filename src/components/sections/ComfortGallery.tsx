...
export default function ComfortGallery() {
  ...
  /* ── Refs for 3D cylinder (desktop only) ── */
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        scrollTimeoutRef.current = setTimeout(() => { scrollingRef.current = false; }, 200);
  ...
  }, [isMobile, applyCylinderTransform, updateOpacities]);
  ...
  /* ── Auto-rotation RAF — desktop only ── */
  useEffect(() => {
    if (isMobile) return;
  ...
  const gradientOverlay = "linear-gradient(to top, rgba(4,10,8,0.97) 0%, rgba(4,10,8,0.72) 28%, transparent 70%)";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ref={headerRef}
        className="container-site relative z-10 flex flex-col gap-4 mb-10 md:mb-14"
      ...
      {/* ══ MOBILE: simple snap-scroll carousel ══ */}
      {isMobile && (
        <div
          role="region"
