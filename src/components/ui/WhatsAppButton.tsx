"use client";

import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { CLINIC } from "@/lib/constants";

export default function WhatsAppButton() {
  const [tabVisible, setTabVisible]   = useState(true);
  const [showTooltip, setShowTooltip] = useState(false);
  const [bounce, setBounce]           = useState(false);

  /* Pause pulse ring when tab is hidden */
  useEffect(() => {
    const handler = () => setTabVisible(document.visibilityState === "visible");
    document.addEventListener("visibilitychange", handler);
    return () => document.removeEventListener("visibilitychange", handler);
  }, []);

  /* Show tooltip on first visit (once) */
  useEffect(() => {
    const seen = sessionStorage.getItem("wa-tooltip-seen");
    if (!seen) {
      const t = setTimeout(() => {
        setShowTooltip(true);
        sessionStorage.setItem("wa-tooltip-seen", "1");
        setTimeout(() => setShowTooltip(false), 4000);
      }, 3000);
      return () => clearTimeout(t);
    }
  }, []);

  /* Bounce every 8 s */
  useEffect(() => {
    const id = setInterval(() => {
      setBounce(true);
      setTimeout(() => setBounce(false), 700);
    }, 8000);
    return () => clearInterval(id);
  }, []);

  return (
    <div className="fixed bottom-24 lg:bottom-6 right-4 lg:right-6 z-50 flex flex-col items-end gap-2">
      {/* Tooltip */}
      <AnimatePresence>
        {showTooltip && (
          <motion.div
            initial={{ opacity: 0, x: 12, scale: 0.92 }}
            animate={{ opacity: 1, x: 0,  scale: 1 }}
            exit={{ opacity: 0, x: 12, scale: 0.92 }}
            transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
            className="flex items-center gap-2 px-4 py-2.5 rounded-full shadow-lg text-sm font-body font-medium text-white cursor-pointer"
            style={{ background: "#1A2421", maxWidth: 210 }}
            onClick={() => setShowTooltip(false)}
          >
            <span>Chat on WhatsApp</span>
            <button
              aria-label="Dismiss"
              className="opacity-50 hover:opacity-100 ml-1 text-xs leading-none"
              onClick={(e) => { e.stopPropagation(); setShowTooltip(false); }}
            >
              ✕
            </button>
          </motion.div>
        )}
      </AnimatePresence>

      {/* Button */}
      <motion.a
        href={CLINIC.whatsapp}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Chat with us on WhatsApp"
        className="relative flex items-center justify-center w-14 h-14 rounded-full shadow-lg"
        style={{ background: "#25D366", willChange: "transform" }}
        initial={{ scale: 0, opacity: 0 }}
        animate={bounce ? { scale: [1, 1.2, 0.95, 1.05, 1], opacity: 1 } : { scale: 1, opacity: 1 }}
        transition={bounce
          ? { duration: 0.6, ease: "easeInOut" }
          : { delay: 2, duration: 0.4, type: "spring" }
        }
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.95 }}
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white" width="28" height="28" aria-hidden="true">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
        </svg>

        {/* Red notification dot */}
        <span
          className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white flex items-center justify-center"
          style={{ background: "#EF4444" }}
          aria-hidden="true"
        >
          <span className="text-white font-bold" style={{ fontSize: 8, lineHeight: 1 }}>1</span>
        </span>

        {/* Pulse ring */}
        {tabVisible && (
          <motion.span
            className="absolute inset-0 rounded-full"
            style={{ background: "#25D366" }}
            animate={{ scale: [1, 1.4, 1.4], opacity: [0.6, 0, 0] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }}
            aria-hidden="true"
          />
        )}
      </motion.a>
    </div>
  );
}
